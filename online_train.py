...
def yield_cycles(df, cycle_inds, cols, blacklist, device):
    ...
    for cycle in range(first_cycle, last_cycle):
        ...
        #df_slice = df_slice.drop(["timestamp","gpsLong","gpsLat","gpsSpeed","gpsQuality"],axis=1)
...
for loop in range(len(train_inds)):
    ...
    lstm_ae, loss_over_time = train_model(lstm_ae, train_tensors, epochs = 100, lr = 1e-4, device = device)
